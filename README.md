...
|---|---|---|
| Application version | `IMPLEMENTED` — **v2.15.1** | `package.json` is the version source; `src/version.js` supplies the label rendered in the app. |
...
The deployed application reports **v2.15.1**, which gives the community portal’s two strength
...
The current application version is **2.15.1**. [`SECURITY.md`](SECURITY.md) is the authority for support and vulnerability-reporting policy; `package.json` is the authority for the application version. Release changes belong in [`CHANGELOG.md`](CHANGELOG.md), avoiding a second release table that can drift.
...
[`CHANGELOG.md`](CHANGELOG.md) is the authoritative release history. The current version is **v2.15.1**.
