...
|---|---|---|
| Application version | `IMPLEMENTED` — **v2.14.0** | `package.json` is the version source; `src/version.js` supplies the label rendered in the app. |
...
| Community functional measures | `IMPLEMENTED` — **v2.14.0** | Grip strength and sit-to-stand are asked in both pathways, banded against published references, and reported on page 3 of the printed report. Optional throughout: neither figure feeds the risk score (`CD20`). `CD21` and `CD23` remain with the owner. |
...
The deployed application reports **v2.14.0**, which adds the community portal’s two strength measurements, the precise-age pathway split and the printed report page that carries them. See [`CHANGELOG.md`](CHANGELOG.md) for the authoritative release record.
...
The current application version is **2.14.0**. [`SECURITY.md`](SECURITY.md) is the authority for support and vulnerability-reporting policy; `package.json` is the authority for the application version. Release changes belong in [`CHANGELOG.md`](CHANGELOG.md), avoiding a second release table that can drift.
...
[`CHANGELOG.md`](CHANGELOG.md) is the authoritative release history. The current version is **v2.14.0**.
...
- [`docs/FUNCTIONAL-MEASURES-ADDIE.md`](docs/FUNCTIONAL-MEASURES-ADDIE.md): the ADDIE record behind the functional measures, **shipped in v2.14.0**. Parts of it were overtaken by what was built and carry `HISTORICAL` / `SUPERSEDED` labels; `CD21` (source wording) and `CD23` (re-measurement) remain with the owner.
