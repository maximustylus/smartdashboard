...
|---|---|---|
| Application version | `IMPLEMENTED` — **v2.16.4** | `package.json` is the version source; `src/version.js` supplies the label rendered in the app. |
...
The deployed application reports **v2.16.4**, which closes the public screening's Firestore
collection to anything but the records the app itself writes (`CP56`). **v2.16.3** before it
is the community portal's release: the
...
The current application version is **2.16.4**. [`SECURITY.md`](SECURITY.md) is the authority for support and vulnerability-reporting policy; `package.json` is the authority for the application version. Release changes belong in [`CHANGELOG.md`](CHANGELOG.md), avoiding a second release table that can drift.
...
| [`docs/FUNCTIONAL-MEASURES-ADDIE.md`](docs/FUNCTIONAL-MEASURES-ADDIE.md) | `IMPLEMENTED` in v2.13.0 and extended in v2.14.0 | The ADDIE record for the grip-strength and sit-to-stand plan. It is a **design document frozen at the point of hand-off**, not a description of what shipped: read `CHANGELOG.md` and `COMMUNITY_TODO.md` for that. Of its nine owner decisions, `CD21` and `CD23` are still open. |
...
[`CHANGELOG.md`](CHANGELOG.md) is the authoritative release history. The current version is **v2.16.4**.
